...
const QUERY_BUILDER = {
  ...
  SAVE: (userId, params) => {
    ...
    return SqlString.format(`INSERT INTO ${COMMENT_TABLE_NAME} SET ${ [COMMENT_FIELDS.ID]} = MD5(RAND()) , ?`, data)
  },
  ...
  LIKE: (commentId, userId) =>{
    ...
    (${COMMENT_THUMB_FIELDS.ID} = MD5(RAND()) , ${COMMENT_THUMB_FIELDS.COMMENT_ID}, ${COMMENT_THUMB_FIELDS.USER_ID}, ${COMMENT_THUMB_FIELDS.STATUS})
    VALUES(?,?,?)
    ON DUPLICATE KEY
